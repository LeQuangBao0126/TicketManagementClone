...
const findIsInviteeInBoard = async (inviteeId, boardId) => {
  try {
    const filter = [{
      $match: {
        $and: [
          { "inviteeId": ObjectId(inviteeId) },
          { 'boardInvitation.boardId': ObjectId(boardId) },
    ...
    const result = await getDB().collection(invitationCollectionName).aggregate(filter).toArray()

    return result.length > 0 ? true : false
  } catch (error) {
    console.log("error in model invitation", error)
    throw new Error(error)
  }
}
    ...
    const results = await getDB().collection(invitationCollectionName).aggregate([
      ...
      },
      {
        $lookup: {
          ...
          foreignField: '_id',
          as: 'invitee', // as là lúc nào cung ra 1 mảng
          pipeline: [
            { $project: { 'password': 0, 'verifyToken': 0 } }
          ]
        }
      },
      {  //sang board model để tìm kiếm theo id
        $lookup: {
          ...
          as: 'board',
          pipeline: [
            { $project: { 'title': 1 } }    // 1 là có , 0 là ko lấy .. chỉ cần lấy title show thôi cho nhẹ
          ]
        }
      }
    ]).toArray()
