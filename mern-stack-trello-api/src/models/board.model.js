      ...
      .aggregate([
        ...
        },
        {
          $lookup: {
            ...
            localField: "ownerIds", //co the la 1 obj hoac 1 array
            foreignField: "_id",
            as: "owners",
            pipeline: [
              { $project: { password: 0, verifyToken: 0 } }, // loai nhung field ko mun tra ve
            ],
          },
        },
        {
          $lookup: {
            from: UserModel.userCollectionName,
            localField: "memberIds", //co the la 1 obj hoac 1 array
            foreignField: "_id",
            as: "members",
            pipeline: [
              { $project: { password: 0, verifyToken: 0 } }, // loai nhung field ko mun tra ve
...
const getListBoards = async (
  ...
  try {
    //tim kiem all board vs match vs dieu kien và : destroy false và ownerIds , hoac memberIds
    let queryConditions = [
      {
        $or: [
    ...
    if (queryFilters) {
      Object.keys(queryFilters).forEach((key) => {
        // muc dich sau khi push xong nhan dc : {title: 'abc' }
